...
I find that the specification of the classes is entirely insufficient in the old version: essentially using the equidistant classes from image, with the trim and equalize modifiers. Ordinarily, there would not be much need for more sophistication than that, but if we want a plausible method for expression sets, we do not want to have to extract the expression matrix, apply a function, pass it in again with the pheno data etc., so having some mechanism at least for the method makes sense. Preferably we want to have some useful shorthands for different types of classes, e.g. equal widths around zero (useful for correlation matrices and residuals, too).

So we have an argument \Rfunarg{breaks} that allows the following specification:
\begin{itemize}
\item a vector containing the actual breaks,
\item a number indicating the number of classes, together with an optional type: either equal widths or equal counts.
\end{itemize}
If unspecified, the number of classes can be calculated using one of the helper functions for \Rfunction{hist} like \Rfunarg{nclass.Sturges}, and the type can depend on the skewness of the data. We wrap this into a service function that returns the classes plus some meta-information that may be useful for plotting the legend.
