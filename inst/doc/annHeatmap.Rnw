%\VignetteIndexEntry{Examples of annotated and regular heatmaps}
%\VignetteDepends{}
...
lm1 = eBayes(lm1)
geneID = topTable(lm1, coef=2, num=100, adjust="none", p.value=0.05)$ID
length(geneID)
@
...
Figure~\ref{fig1} shows the full example data with the default settings. Figure~\ref{fig2} shows the gene expression for the short list, with the legend move to the right, for simpler breaks of the intensity scale and a different palette. Figure~\ref{fig3} shows how different distance- and clustering functions can be passed in. Figure~\ref{fig4} shows how the lists of arguments can be used to specify different settings for row- and column dendrograms.
...
reg3 = regHeatmap(exprs(exdat2), legend=2, dendrogram=list(clustfun=hclust.avl, distfun=corrdist))
plot(reg3)
@
\end{center}
...
reg4 = regHeatmap(exprs(exdat2), legend=3, dendrogram=list(Col=list(status="hide")))
plot(reg4)
@
\end{center}
\caption{Heatmap with row- and column dendrograms and a legend for 46 genes and 26 samples. The column dendrogram is not shown, though the samples are still arranged as in Figure~\ref{fig2}.\label{fig4}}
\end{figure}
}}}
...
@
\end{center}
\caption{Annotated heatmap with row- and column dendrograms and a legend for 500 genes and 26 samples.\label{fig5}}
\end{figure}
}}}
...
@
\end{center}
\caption{Annotated heatmap with row- and column dendrograms and a legend for 46 genes and 26 samples. The column dendrogram is cut at $h=5000$\label{fig6}}
\end{figure}
}}} 
...
@
Finally, we want to see which of the features are associated with the GeneOntology category \textit{translation}:
<<setupX3>>=
require(hgu95av2.db)
allGO = as.list(mget(featureNames(exdat2), hgu95av2GO))
isTranslation = sapply(allGO, function(x) "GO:0006412" %in% names(x))
@
Let's put this into an annotation data frame:
<<setupX4>>=
annFeatures = data.frame(standard.errors=SE, sigCorScore=PV<0.05, isTranslation)
@
...
ann4 = annHeatmap2(exprs(exdat2), ann=list(Row=list(data=pData(exdat2)), Col=list(data=annFeatures)))
plot(ann4)
@
\end{center}
